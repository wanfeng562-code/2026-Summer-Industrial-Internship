...
public class AiConfig {
    ...
    public ChatClient chatClient(ChatClient.Builder builder, TicketAiTools ticketAiTools){
        return builder.defaultSystem("""
                ...
                - 用户查询全部订单时必须调用“查询当前登录用户有权访问的订单列表”工具
                - 用户查询全部工单时必须调用“查询当前登录用户有权访问的工单列表”工具
                - 用户提供 ORD 开头的订单号时调用订单查询工具；提供 TK 开头的工单号或数字工单ID时调用工单查询工具
                - 读取业务数据必须使用工具返回的真实结果；需要执行写操作时，明确提示用户进入工单或转人工
