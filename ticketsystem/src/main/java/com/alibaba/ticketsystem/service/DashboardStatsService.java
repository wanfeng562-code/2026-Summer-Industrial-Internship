...
public class DashboardStatsService {
    ...
    public DashboardStatsVo currentScopeStats() {
        ...
        for (String category : CATEGORIES) {
            categories.put(category, tickets.stream()
                    .filter(ticket -> category.equals(ticket.getCategory())).count());
        }
    ...
    private QueryWrapper<Ticket> scopeQuery(SysUser user) {
        ...
        return switch (user.getRole()) {
            ...
            case "AGENT" -> query.and(wrapper -> wrapper
                    .eq("agent_id", user.getId())
                    .or(nested -> nested.isNull("agent_id").eq("status", TicketStatus.MANUAL_REVIEW.name())));
            case "ADMIN" -> query;
