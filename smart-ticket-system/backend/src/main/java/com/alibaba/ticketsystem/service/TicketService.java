...
public class TicketService {
    ...
    public TicketVo getTicketDetail(Long ticketId){
        ...
        if(ticket == null){
            throw new ApiException(404, "该工单不存在");
        }
        assertTicketReadable(ticket);
        return  convertToTicketVo(ticket);
    }

    /** 普通用户仅可查看自己的工单；客服/管理员可查看全量 */
    public void assertTicketReadable(Ticket ticket) {
        Long loginId = StpUtil.getLoginIdAsLong();
        SysUser current = sysUserMapper.selectById(loginId);
        if (current == null) {
            throw new ApiException(401, "未登录或登录已失效");
        }
        if ("USER".equals(current.getRole()) && !loginId.equals(ticket.getUserId())) {
            throw new ApiException(403, "无权查看他人工单");
        }
    }
    ...
    public TicketVo createTicket(TicketCreateRequest ticketCreateRequest){
        ...
        //1.校验订单存在且属于当前登录用户
        Orders orders = ordersService.assertOrderOwnedByCurrentUser(ticketCreateRequest.getOrdersId());
