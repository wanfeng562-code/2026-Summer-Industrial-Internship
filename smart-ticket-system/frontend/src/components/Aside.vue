...
const menus = computed(() => {
  ...
  const all = [
    ...
    { path: '/home/create', title: '创建工单', icon: 'Plus', roles: ['USER', 'ADMIN'] },
    { path: '/home/orders', title: '订单管理', icon: 'ShoppingCart', roles: ['USER', 'AGENT', 'ADMIN'] },
    { path: '/home/policies', title: '售后策略', icon: 'Setting', roles: ['ADMIN'] },
    { path: '/home/chat', title: 'AI客服', icon: 'ChatDotRound', roles: ['USER', 'AGENT', 'ADMIN'] },
  ]
  return all.filter((m) => m.roles.some((r) => roles.includes(r)))
})
</script>
